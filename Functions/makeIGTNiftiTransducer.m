...
% pcds (elem x (xyz)) - for IGT transducer 
...
usf = 4; % grid usf - sets the voxel size (0.25) = 1/4 
padding = 30; % padding in all 3 dim
zpaddingextra = 30; % optional extra z padding - for more visual at focus 
...
% shift P - P are the PCD elements 
P = P.*usf;
P(:,1) = P(:,1) + fL(1);
P(:,2) = P(:,2)+fL(2);
P = round(P);
...
%% Don't know what this is for exactly, but could be useful one day 
